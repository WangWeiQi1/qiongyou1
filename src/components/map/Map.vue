<template>
  <div>
    <div>
      <span :offset="2" :span="8">
        <input :id="suggestId" v-model="address_detail" :clearable='clearable' placeholder="请输入店铺地址,获取店铺坐标" >

      </span>
      <span :span="2">
        <button id="position" @click="search" type="primary">定位</button>
      </span>
      <span :span="12" >
        <div type="success" v-clipboard:copy="userlocation.lng" v-clipboard:success="onCopy" v-clipboard:error="onError" >经度 {{userlocation.lng}}</div>
        <div type="success" v-clipboard:copy="userlocation.lat" v-clipboard:success="onCopy" v-clipboard:error="onError">纬度 {{userlocation.lat}}</div>

      </span>
    </div>
    <div>
      <span :offset="2" :span="18">
